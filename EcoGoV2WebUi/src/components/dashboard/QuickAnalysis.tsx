...
const QuickAnalysis: React.FC = () => {
  ...
  const { analyzeInvoice, loading } = useApi();

  const onDrop = useCallback((acceptedFiles: File[]) => {
    const newFiles = acceptedFiles.map(file => ({
      file,
      type: 'fuel' as 'fuel' | 'plane' | 'train',
  ...
  return (
    <div className="bg-white rounded-lg shadow-card p-6">
      <h2 className="text-xl font-semibold text-gray-900 mb-6">Quick Carbon Analysis</h2>
          ...
          <div className="flex flex-col items-center justify-center space-y-2">
            ...
            <p className="text-sm font-medium text-gray-700">
              {isDragActive 
                ...
                : 'Analyze invoices instantly - no trip required'
              }
            </p>
        ...
        {files.length > 0 && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end">
              ...
              >
                {uploadStatus === 'uploading' ? 'Analyzing...' : 'Analyze Invoices'}
              </button>
            </div>
          </div>
        )}
